...
import '../../../../domain/usecase/user/login_with_token.dart';
...
class UserBloc extends Bloc<UserEvent, UserState> {
  ...
  Future<void> _onUserLogin(
    ...
    try {
      ...
      final User? user = await _userUsecase.excute(usecase: LoginUsecase());
      if (user == null) {
        //에러 상황은 아니야
  ...
  Future<void> _onUserLoginWithToken(
    ...
    try {
      emit(state.copyWith(status: Status.loading));
      final User? user = await _userUsecase.excute(usecase: LoginWithTokenUsecase());
      if (user == null) {
        //에러 상황은 아니야
        emit(state.copyWith(status: Status.initial));
      } else {
  ...
  Future<void> _onUserLogout(
    ...
    try {
      emit(state.copyWith(status: Status.loading));
      final User? user = await _userUsecase.excute(usecase: LogoutUsecase());
